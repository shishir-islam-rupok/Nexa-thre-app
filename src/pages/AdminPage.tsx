...
import { Shield, Users, CreditCard, Flag, Search, CheckCircle2, XCircle, Activity, DollarSign } from 'lucide-react';
...
export default function AdminPage() {
  ...
  if (!profile || !['admin', 'moderator'].includes(profile.role)) {
    return <div className="max-w-xl mx-auto px-4 py-16"><EmptyState icon={<Shield />} title="Access denied" description="Only admins and moderators can open the control center." /></div>;
  }

  const pending = tx.filter((x) => x.status === 'pending');
  const pendingReports = reports.filter((x) => x.status === 'pending');
  ...
  async function reviewPayment(item: Transaction, status: 'approved' | 'rejected') {
    if (profile.role !== 'admin') {
      showToast('Only admins can approve payments', 'error');
      return;
    }
    setBusy(item.id);
    const { error } = await supabase.rpc('review_payment', {
      ...
      p_status: status,
      p_admin_note: status === 'rejected' ? 'Rejected by admin' : 'Approved by admin',
    });
    if (error) showToast(error.message, 'error');
    else { showToast(`Payment ${status}`); await load(); }
    setBusy(null);
  }
  ...
  return <div className="max-w-6xl mx-auto px-4 py-6 space-y-5">
    <header className="flex items-center justify-between">
      <div className="flex items-center gap-2"><div className="w-9 h-9 rounded-xl bg-accent-600 text-white flex items-center justify-center"><Shield className="w-5 h-5" /></div><div><h1 className="text-xl font-bold">NEXA Control Center</h1><p className="text-xs text-ink-400 flex items-center gap-1"><Activity className="w-3 h-3 text-emerald-500" /> Atomic payments · Live moderation</p></div></div>
      <span className="px-2.5 py-1 rounded-full bg-accent-50 dark:bg-accent-900/20 text-accent-600 text-xs font-semibold uppercase">{profile.role}</span>
    </header>
...
function Status({ status }: { status: string }) {
  const ok = ['approved', 'completed'].includes(status);
  const bad = ['rejected', 'failed', 'cancelled'].includes(status);
  return <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${ok ? 'bg-emerald-50 text-emerald-600' : bad ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'}`}>{status}</span>;
}
