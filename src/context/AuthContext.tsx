...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function ensureProfile(u: User) {
    ...
    const fullName = meta.full_name || meta.name || 'New User';
    const username = meta.preferred_username || meta.name?.toString().toLowerCase().replace(/[^a-z0-9]/g, '') || `user${u.id.slice(0, 8)}`;
    const avatarUrl = meta.avatar_url || meta.picture || '';

    await supabase
      .from('profiles')
      .insert({ id: u.id, username, full_name: fullName, avatar_url: avatarUrl });
  }
  ...
  return (
    <AuthContext.Provider value={{
      ...
      signUp, signIn, signInWithGoogle, signInWithPhone, sendOtp, verifyOtp,
      resetPassword, updatePassword, deleteAccount, signOut, refreshProfile,
    }}>
      {children}
